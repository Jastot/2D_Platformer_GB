...
namespace PatrollAI
{
    public class ProtectorAI: IProtector
    {
        ...
        public ProtectorAI(PlayerView view, PatrolAIModel model, AIDestinationSetter destinationSetter, AIPatrolPath patrolPath)
        {
        ...
        private void OnTargetReached(object sender, EventArgs e)
        {
            _destinationSetter.target = _isPatrolling
                ...
                : _model.GetClosestTarget(_view.Struct.Transform.position);
        }
        ...
        {
            _isPatrolling = true;
            _destinationSetter.target = _model.GetClosestTarget(_view.Struct.Transform.position);
        }
